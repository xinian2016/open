...
echo "$CopyrightLogo";
echo 
echo "一键CentOS搭建openVPN免流[文本密码验证版]"
echo "请按回车继续开始安装："
read
echo 
# 
...
rpm -ivh https://raw.github.com/mu228/pwd/master/64-epel-release-5-4.noarch.rpm
else
rpm -ivh https://raw.github.com/mu228/pwd/master/32-epel-release-5-4.noarch.rpm
fi
fi

if [ $version == "6" ];then
if [ $(getconf LONG_BIT) = '64' ] ; then
rpm -ivh https://raw.github.com/mu228/pwd/master/epel-release-6-8.noarch.rpm
else
rpm -ivh https://raw.github.com/mu228/pwd/master/32-epel-release-6-8.noarch.rpm
fi
fi

if [ $version == "7" ];then
rpm -ivh https://raw.github.com/mu228/pwd/master/epel-release-latest-7.noarch.rpm
fi
...
cd /etc/
rm -rf ./sysctl.conf
wget https://raw.github.com/mu228/pwd/master/sysctl.conf
sleep 3
...
cd /etc/openvpn/
rm -rf ./server.conf
wget https://raw.github.com/mu228/pwd/master/server.tar.gz
tar -zxf server.tar.gz
cd /etc/squid/
rm -f ./squid.conf
wget https://raw.github.com/mu228/pwd/master/squid.conf
...
rm -rf ./{ta.key,myip,ovpn.1,ovpn.2,ovpn.3,ovpn.4,ovpn.5,ovpn.6,ovpn.7,ovpn.8,ovpn.9,ovpn.10,ovpn.11,ovpn.12,ovpn.13,ovpn.14,ovpn.15,ovpn.16,me.ovpn,ca.crt,me.{crt,key}}
clear
# OpenVPN Installing ****************************************************************************
echo 
echo "正在创建下载链接："
